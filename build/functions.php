...
function getTermsList( $taxonomy ) {
  ...
  if ( !empty( $terms ) && !is_wp_error( $terms ) ){
      ...
      foreach ( $terms as $term ) {
        echo '<li><a href="' . get_permalink() . '?job_category=' .$term->term_id . '">' . $term->name . '&nbsp;(' . $term->count . ')' . '</a></li>';
      }
      echo '</ul>';
  }
}

function add_parameter_to_permalink( $key, $value, $permalink = ''){
  if( strlen( $permalink ) === 0) {
    $permalink = get_current_url();
  }

  $permalink = preg_replace( '/[?&]' . $key . '=\d+/', '', $permalink);
...
function clear_filter_var_from_url($permalink, $key) {
  return preg_replace( '/[?&]' . $key . '=\d+/', '', $permalink);
}
